...
public class MenuColumnSupport
{
	...
	public void setColumn(MenuColumnSupport su)
	{
		...
		column1.setMinimumSize(new Dimension(195,70));
		column1.setPreferredSize(new Dimension(300,100));
		column1.setMaximumSize(new Dimension(400,260));
	...
	{	
		...
		cs << new MenuColumnSupport()
		MenuColumnSupport.loadMenu(cs, "./data/stylesheets.txt", true)      
		// ?????????????????????????????????????????????????????
